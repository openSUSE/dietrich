...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!-- I could use just "para", but let's try to avoid using this template as
  much as possible. Might even help performance despite the monster entity. -->
  <xsl:template match="para[child::*[(&dbselfblocks;)]]">
  ...
  <xsl:template match="para/*|para/text()">
  ...
  <xsl:template match="para/*|para/text()" mode="copy">
    <xsl:if test="starts-with(..,'The following illustration')">
      <xsl:message>unmi cat : <xsl:value-of select="local-name(.)"/> | <xsl:value-of select="."/> / <xsl:value-of select="@*"/></xsl:message>
    </xsl:if>
    <xsl:copy-of select="."/>
    <xsl:choose>
    <xsl:when test="not(following-sibling::node()[1][&dbselfblocks;])">
     <xsl:message>when <xsl:value-of select="local-name(following-sibling::node()[1])"/></xsl:message>
    </xsl:when>
    <xsl:otherwise>
    <xsl:message>othe <xsl:value-of select="local-name(following-sibling::node()[1])"/></xsl:message>
    </xsl:otherwise>
    </xsl:choose>
