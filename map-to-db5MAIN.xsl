...
<xsl:stylesheet version="1.0" xmlns="http://docbook.org/ns/docbook"
 ...
 <xsl:template match="/">
  ...
  <xsl:apply-templates
   select="node()[normalize-space()][1][self::processing-instruction()]"/>
 ...
 <xsl:template match="*" priority="0">
  <xsl:message>WARNING: Unhandled element <xsl:value-of select="local-name(.)"
   />, ignored.</xsl:message>
 </xsl:template>
 ...
 <xsl:template
  match="bookmeta | bookrights | bookid | series | component | brand | prodname | vrm"/>
 ...
 <xsl:template match="appendix | chapter | preface | topicref">
 ...
 <xsl:template match="@href" mode="collect.files">
  ...
  <xsl:variable name="removal">
   <xsl:if test="contains($remove, concat(' ', normalize-space(.), ' '))"
    >1</xsl:if>
  </xsl:variable>
  <xsl:variable name="replacement">
   <xsl:if test="contains($replace, concat(' ', normalize-space(.), '='))">
    <xsl:value-of
     select="substring-before(substring-after($replace, concat(' ', normalize-space(.), '=')), ' ')"
    />
   </xsl:if>
  </xsl:variable>
 ...
 <xsl:template name="changeroot">
  ...
  <xsl:variable name="root">
   <xsl:choose>
    ...
    <xsl:otherwise>
     <xsl:message>WARNING: Unhandled element <xsl:value-of
       select="local-name(.)"/> (with @href), replaced with
      section.</xsl:message>
     <xsl:text>section</xsl:text>
    </xsl:otherwise>
   </xsl:choose>
  </xsl:variable>
  <xsl:message>file:<xsl:value-of select="$file"/>,root:<xsl:value-of
    select="$root"/></xsl:message>
 </xsl:template>
